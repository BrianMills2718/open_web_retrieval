...
| Medium-specific retrieval: topic search scoping, member-paywall full-text fetch ladder, and full-text RSS feeds | `open_web_retrieval` | `open_web_retrieval` | shared infrastructure | bounded site adapter | First site-specific adapter; built on the shared search/fetch/extract primitives. If more site adapters appear, consider a sub-package. |
